...
_known_attributes = [
    ...
    ("rsync_cmd", attr.string, dict(
        default = "rsync --files-from={filelist}",
        doc = """\
Command to run in order to execute rsync - can refer to system binaries (non-hermetic) or binaries brought in via the 'tools' attribute.
It is a string template expanded using .format(), with the only variable provided being 'filelist', the name of the file containing
...
no ssh command is invoked - if the target is executable, it is run directly.""")),
    ...
    ("all_files", attr.bool, dict(default = False, doc = """\
By default, this rule only copies the runfiles needed by the specified target.
If set to true, it will instead copy ALL the files the target generates.""")),
]
...
def _export_and_run_impl(ctx):
    ...
    subs = dict(
        ...
        rsync_cmd = shell.quote(attrs.rsync_cmd.format(
            filelist = shell.quote(include.short_path))),
